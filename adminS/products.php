...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_product'])) {
    ...
    if (empty($productName) || empty($productCategory) || empty($productPrice) || empty($productBarcode)) {
        ...
        $stmt = $conn->prepare("INSERT INTO products (name, category, price, stock, description, image_path, barcode, discount_type, discount_value, featured) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssidssssdi", $productName, $productCategory, $productPrice, $productStock, $productDescription, $targetFile, $productBarcode, $discountType, $discountValue, $featured);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_product'])) {
    ...
    if (empty($productId) || empty($productName) || empty($productCategory) || empty($productPrice) || empty($productBarcode)) {
        ...
        if (!empty($productImage)) {
            ...
            $stmt = $conn->prepare("UPDATE products SET name = ?, category = ?, price = ?, stock = ?, description = ?, image_path = ?, barcode = ?, discount_type = ?, discount_value = ?, featured = ? WHERE id = ?");
            $stmt->bind_param("ssidssssdi", $productName, $productCategory, $productPrice, $productStock, $productDescription, $targetFile, $productBarcode, $discountType, $discountValue, $featured, $productId);
        } else {
            // Update without changing the image
            $stmt = $conn->prepare("UPDATE products SET name = ?, category = ?, price = ?, stock = ?, description = ?, barcode = ?, discount_type = ?, discount_value = ?, featured = ? WHERE id = ?");
            $stmt->bind_param("ssidsssdi", $productName, $productCategory, $productPrice, $productStock, $productDescription, $productBarcode, $discountType, $discountValue, $featured, $productId);
        }
...
$recentProductsQuery = "SELECT id, name, category, price, stock, description, image_path, barcode, discount_type, discount_value, featured FROM products ORDER BY created_at DESC LIMIT 30";
...
/* Checkbox styling for product management */
.checkbox-label {
    font-size: 0.9em;
    color: #666;
    margin-left: 5px;
...
/* Improve checkbox group alignment and spacing */
.form-group.checkbox-group {
    display: flex;
    align-items: center;
    margin-bottom: 15px;
...
/* Add a subtle highlight for the featured checkbox */
...
.form-group.checkbox-group input[type="checkbox"]:checked + .checkbox-label {
    color: #4caf50;
    font-weight: 500;
}
                        ...
                        <div class="form-grid">
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" id="product_stock" name="product_stock" class="form-control" placeholder="Enter stock quantity" min="0" 
                                    value="<?php echo $editProduct ? htmlspecialchars($editProduct['stock']) : (isset($productStock) ? htmlspecialchars($productStock) : '0'); ?>">
                            </div>
                            ...
                            <div class="form-group checkbox-group">
                                ...
                                <input type="checkbox" id="featured" name="featured" <?php echo ($editProduct && $editProduct['featured'] == 1) ? 'checked' : ''; ?>>
                                <span class="checkbox-label">Display this product on the homepage</span>
                            </div>
                ...
                <form id="editProductForm" enctype="multipart/form-data">
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" id="edit_product_stock" name="product_stock" class="form-control" placeholder="Enter stock quantity" min="0" value="0">
                        </div>
                ...
                <div class="product-card preview-card">
                    ...
                    <div class="product-details">
                        ...
                        <div class="product-featured">
                            <strong>Featured:</strong> <span id="fullPreviewFeatured"></span>
                        </div>
                    </div>
                </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            tabs.forEach(tab => {
                ...
                function updatePreview() {
                    ...
                    const featured = document.getElementById('edit_featured').checked ? 'Yes' : 'No';
                    ...
                    document.getElementById('fullPreviewFeatured').textContent = featured;
                ...
                document.getElementById('editProductForm').addEventListener('submit', function(e) {
                    ...
                    formData.append('featured', document.getElementById('edit_featured').checked ? 1 : 0);

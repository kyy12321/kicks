...
$searchCondition = '';
if (!empty($search)) {
    $searchCondition = " AND (users.username LIKE '%$search%' OR orders.order_id LIKE '%$search%')";
}
...
$orderQuery = "SELECT orders.order_id, users.username, orders.order_date, 
...
$countStatusQuery = "SELECT LOWER(status) AS lower_status, COUNT(*) as count FROM orders GROUP BY lower_status";
$statusResult = $conn->query($countStatusQuery);

if ($statusResult) {
    while ($row = $statusResult->fetch_assoc()) {
        $statusKey = ucfirst($row['lower_status']);
        if (array_key_exists($statusKey, $statusCounts)) {
            $statusCounts[$statusKey] = $row['count'];
                ...
                <!-- Search Form -->
                <form method="GET" action="orders.php" class="search-container">

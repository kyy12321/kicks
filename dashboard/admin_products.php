...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_product'])) {
    ...
    if (empty($productName) || empty($productCategory) || empty($productPrice) || empty($productBarcode)) {
        ...
        $stmt = $conn->prepare("INSERT INTO products (name, category, price, stock, description, image_path, barcode, discount_type, discount_value, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
...
$recentProductsQuery = "SELECT id, name, category, price, stock, description, image_path, discount_type, discount_value FROM products ORDER BY created_at DESC LIMIT 100";
...
<body>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Product search functionality with AJAX
            ...
            function handleSearchResults(products) {
                productList.innerHTML = ''; // Clear current results
                ...
                products.forEach(product => {
                    ...
                    const hasDiscount = finalPrice < parseFloat(product.price);
            ...
            // Function to initialize event listeners for edit/delete buttons
            function initEditDeleteListeners() {
                ...
                document.querySelectorAll('.product-action-btn.delete').forEach(button => {
                    button.addEventListener('click', function() {
                        const productId = this.getAttribute('data-id');
                        const productName = this.getAttribute('data-name');
                        ...
                        // Store current product ID for deletion
            ...
            productSearchForm.addEventListener('submit', function(e) {
                ...
                productList.innerHTML = '<p class="loading">Searching products...</p>';
                ...
                fetch(`search_products.php?search=${encodeURIComponent(searchTerm)}`)
                    ...
                    .then(data => {
                        if (data.status === 'success') {
                            handleSearchResults(data.products);
                        } else {
                            alert('Search failed: ' + data.message);
                            productList.innerHTML = '<p class="no-results">Search failed. Please try again.</p>';
                        }
                    })
                    .catch(error => {
                        console.error('Search error:', error);
                        productList.innerHTML = '<p class="no-results">Error performing search. Please try again.</p>';
                    });
            });
